...
def gauss_sample_z(logits, zsize):
    U = torch.randn(logits.size(0), zsize)
    x = torch.zeros(logits.size(0), 1, zsize)
...
def gauss_sample_z_new(logits, zsize):
    U = torch.randn(logits.size(0), logits.size(1), zsize)
    x = torch.zeros(logits.size(0), logits.size(1), zsize)
    x[:, :, :] = U[:, :, :] + logits[:, :, 0:zsize]
    return x
...
def preprocess_adj_new(adj):
    adj_normalized = torch.eye(adj.shape[0], device=adj.device) - (adj.transpose(0, 1))
    return adj_normalized


def preprocess_adj_new1(adj):
    adj_normalized = torch.inverse(
        torch.eye(adj.shape[0], device=adj.device) - adj.transpose(0, 1)
    )
    return adj_normalized
...
def matrix_poly(matrix, d):
    x = torch.eye(d, device=matrix.device) + torch.div(matrix, d)
...
def load_forbidden_edges(config):
    """
    Loads forbidden edges from prior knowledge and converts them to a mask matrix

    Args:
        config: Configuration object containing column_names
        
    Returns:
        torch.DoubleTensor: Binary mask matrix where 0 indicates forbidden edges
    """
    # Initialize mask matrix with all ones
    mask = np.ones((config.num_feature_nodes, config.num_feature_nodes))

    try:
        # Read forbidden edges file using CONFIG
        forbidden_df = pd.read_csv(os.path.expanduser(config.forbidden_edges_filepath))

        # Create dictionary mapping variable names to indices
        var_to_idx = {name: idx for idx, name in enumerate(config.feature_names)}

        # Process each forbidden edge
        for _, row in forbidden_df.iterrows():
            ...
            if source in var_to_idx and target in var_to_idx:
                ...
                print(f"Warning: Edge {source}->{target} references unknown variable(s) or variable(s) not in subset")

        print(f"Loaded {len(forbidden_df)} forbidden edges from prior knowledge")

    except FileNotFoundError:
        print("No forbidden edges file found. Using fully connected graph as prior.")
    except Exception as e:
        print(f"Error loading forbidden edges: {str(e)}")
        print("Using fully connected graph as prior.")

    return torch.DoubleTensor(mask)


def load_required_edges(config, positive=True):
    """
    Load positive required edges mask
    """
    prefix = "positive" if positive else "negative"
    filepath = config.positive_required_edges_filepath if positive else config.negative_required_edges_filepath
    mask = np.zeros((config.num_feature_nodes, config.num_feature_nodes))
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if source in var_to_idx and target in var_to_idx:
                ...
                print(f"Warning: {prefix.capitalize()} edge {source}->{target} references unknown variable(s) or variable(s) not in subset")

        print(f"Loaded {len(df)} {prefix} required edges")
        return torch.DoubleTensor(mask)

    except FileNotFoundError:
        print(f"No {prefix} required edges file found.")
    except Exception as e:
        print(f"Error loading {prefix} required edges: {str(e)}")
    return None


def enforce_edge_constraints(A, forbidden_edges, positive_required, negative_required, graph_threshold):
    # First apply forbidden edges
    A.data = A.data * forbidden_edges.to(A.device)

    if positive_required is not None:
        # Handle positive required edges
        min_positive = graph_threshold * 1.2
        positive_required = positive_required.to(A.device)
        A.data = A.data * (1 - positive_required) + \
                torch.max(A.data, min_positive * torch.ones_like(A.data)) * positive_required

    if negative_required is not None:
        # Handle negative required edges
        max_negative = -graph_threshold * 1.2
        negative_required = negative_required.to(A.device)
        A.data = A.data * (1 - negative_required) + \
                torch.min(A.data, max_negative * torch.ones_like(A.data)) * negative_required
